...
float initial_cutset = 0, cutset = 0;
...
void re_cal_DS()
{
	...
	net_card.erase(net_card.begin(), net_card.end());
...
int main(int argc, char** argv)
{
	...
	{
		reqnet = t->first; 
		reqnetcount = reqnetcount + 1;
		for (int g = 0; g < net_node_map[reqnet].size(); g++)
		{
			if (((reqnetcount) % 2) == 0)
			{
				node_map[net_node_map[reqnet][g]][1] = 1;
			}
			else if (((reqnetcount) % 2) != 0)
			{
				node_map[net_node_map[reqnet][g]][1] = -1;
			}
		}
	...
	for (n_v n = node_map.begin(); n != node_map.end(); n++)
	{
		if (node_map[n->first][1] == 1)
		{
			wp1 = wp1 + node_map[n->first][0];
		}
		else if (node_map[n->first][1] == -1)
		{
			wpn1 = wpn1 + node_map[n->first][0];
		}
	}
	...
	end:int stop_s = clock(); //Timing of code ends here
