...
void re_cal_DS()
{
	...
	//fixed_cell_list.erase(fixed_cell_list.begin(), fixed_cell_list.end());
...
int main(int argc, char** argv)
{
	...
	//random partitioning
	for (netnodeiter iter = net_node_map.begin(); iter != net_node_map.end(); iter++)
	{
		for(int j=0; j < net_node_map[iter->first].size(); j++)
		{
			if(((count_n/10)%2) == 0)
			{
				node_map[net_node_map[iter->first][j]][1] = 1;
			}
			else
			{
				node_map[net_node_map[iter->first][j]][1] = -1;
